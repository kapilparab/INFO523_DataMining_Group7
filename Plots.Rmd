...
```{r}
View(vehicles %>% select(VEH_MOD_YEAR) %>% distinct())
```
...
airbag_data <- vehicles %>%
  ...
  select(IMPACT_LOC, AIRBAG_DEPLOYED, AIRBAG_LOC, INJURY) %>%
  mutate(AIRBAG_LOC = case_when(
    AIRBAG_LOC == "Curtain (roof)" ~ "Curtain",
    AIRBAG_LOC == "Other (Knee, air belt, etc.)" ~ "Rear Curtain etc",
...
# Calculate overall probabilities of airbag deployment and survival
overall_prob <- airbag_data %>%
  summarise(
    prob_airbag_deployed = mean(AIRBAG_DEPLOYED == "Yes"),
    prob_survival = mean(SURVIVAL)
  )

conditional_prob <- airbag_data %>%
  filter(AIRBAG_DEPLOYED == "Yes") %>%
...
ggplot(conditional_prob, aes(x = AIRBAG_LOC, y = prob_survival_given_airbag)) +
...
ggsave("airbag_surv.png", path = "plots")
```
